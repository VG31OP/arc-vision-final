#!/usr/bin/env python3
"""Analyze keyframe and timestamp structure of ARC VISION recording segments.
...
Typical use:
    # Inside the ARC VISION container (or wherever recordings are mounted)
...
def resolve_ffprobe_path(override: str | None) -> str:
    ...
    if version:
        ...
        if Path(path).is_file():
            return path
    # Fall back to scanning the ARC VISION ffmpeg install root.
...
def find_segments_near_timestamp(
    ...
    # should contain the timestamp (ARC VISION catalogs segments by filename).
    target_idx = -1
...
def analyze(ffprobe: str, segment: Path, highlight: bool = False) -> None:
    ...
    # (Filename time is what ARC VISION uses as recording.start_time in the DB.)
