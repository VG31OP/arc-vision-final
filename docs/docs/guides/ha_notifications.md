...
The best way to get started with notifications for ARC VISION is to use a Home Assistant mobile notification automation or blueprint example that matches your device and notification flow. ARC VISION's MQTT interface remains compatible with the older MQTT topic model used by shared community tooling, so community notification examples can often be adapted as a starting point. You can use the generated YAML from a Home Assistant blueprint as a starting point and customize it for your ARC VISION instance.
...
Here is a simple example of a notification automation of tracked objects which will update the existing notification for each change. This means the image you see in the notification will update as ARC VISION finds a "better" image.
...
```yaml
automation:
  - alias: Security_ARC_VISION_Notifications
    description: ""
    trigger:
      - platform: mqtt
