...
This guide outlines the basic configuration steps needed to set up a reverse proxy in front of your ARC VISION instance.

A reverse proxy is typically needed if you want to set up ARC VISION on a custom URL, on a subdomain, or on a host serving multiple sites. It could also be used to set up your own authentication provider or for more advanced HTTP routing.

Before setting up a reverse proxy, check if any of the built-in functionality in ARC VISION suits your needs:
|Topic|Docs|
|-|-|
...
**Note about TLS**
When using a reverse proxy, the TLS session is usually terminated at the proxy, sending the internal request over plain HTTP. If this is the desired behavior, TLS must first be disabled in ARC VISION, or you will encounter an HTTP 400 error: "The plain HTTP request was sent to HTTPS port."
To disable TLS, set the following in your ARC VISION configuration:
...
Make life easier for yourself by presenting your ARC VISION interface as a DNS sub-domain rather than as a sub-folder of your main domain.
Here we access ARC VISION via https://cctv.mydomain.co.uk
...
<VirtualHost *:443>
    ...
    ProxyPass "/"  "http://arcvisionpi.local:8971/"
    ProxyPassReverse "/"  "http://arcvisionpi.local:8971/"

    ProxyPass /ws ws://arcvisionpi.local:8971/ws
    ProxyPassReverse /ws ws://arcvisionpi.local:8971/ws

    ProxyPass /live/ ws://arcvisionpi.local:8971/live/
    ProxyPassReverse /live/ ws://arcvisionpi.local:8971/live/

    RewriteEngine on
    RewriteCond %{HTTP:Upgrade} =websocket [NC]
    RewriteRule /(.*)  ws://arcvisionpi.local:8971/$1 [P,L]
    RewriteCond %{HTTP:Upgrade} !=websocket [NC]
    RewriteRule /(.*)  http://arcvisionpi.local:8971/$1 [P,L]
</VirtualHost>
```

### Step 2: Use SSL to encrypt access to your ARC VISION instance

Whilst this won't, on its own, prevent access to your ARC VISION webserver it will encrypt all content (such as login credentials).
...
```
# ------------------------------------------------------------
# arc-vision.domain.com
# ------------------------------------------------------------

server {
  set $forward_scheme http;
  set $server         "192.168.100.2"; # ARC VISION SERVER LOCATION
  set $port           8971;
  ...
  server_name arc-vision.domain.com;
}
```
...
This example shows how to add a `label` to the ARC VISION Docker compose file, enabling Traefik to automatically discover your ARC VISION instance.
...
services:
  arc-vision:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.services.arc-vision.loadbalancer.server.port=8971"
      - "traefik.http.routers.arc-vision.rule=Host(`traefik.example.com`)"
```
...
Note that with this approach, you don't need to expose any ports for the ARC VISION instance since all traffic will be routed over the internal Docker network.

## Caddy Reverse Proxy

This example shows ARC VISION running under a subdomain with logging and a tls cert (in this case a wildcard domain cert obtained independently of caddy) handled via imports
...
arc-vision.YOUR_DOMAIN.TLD {
        reverse_proxy http://localhost:8971
        import tls
        import logging arc-vision.YOUR_DOMAIN.TLD
}
