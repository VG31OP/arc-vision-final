...
ARC VISION is designed to run locally and does not require a persistent internet connection for core functionality. However, certain features need internet access for initial setup or ongoing operation. This page describes what connects to the internet, when, and how to control it.

## How ARC VISION Uses the Internet

ARC VISION's internet usage falls into three categories:
...
2. **Optional cloud services**: Features like the Plus model service and Generative AI connect to external APIs only when explicitly configured.
...
After initial setup, ARC VISION can run fully offline as long as all required models have been downloaded and no cloud-dependent features are enabled.
...
The MobileNetV2 base weights are the one exception to the `/config/model_cache/` rule. They are also the only entry that is not downloaded when the feature is enabled: ARC VISION fetches them when a training run actually starts.
...
If you have already downloaded all required models and want to prevent ARC VISION from attempting any outbound connections to HuggingFace or the Transformers library, set the following environment variables on your ARC VISION container:
...
If your ARC VISION instance has restricted internet access, you can point model downloads at internal mirrors using environment variables:
...
ARC VISION checks GitHub for the latest release version on startup by querying `https://api.github.com`. This can be disabled:
...
When [notifications](/configuration/notifications) are enabled and users have registered for push notifications in the web UI, ARC VISION sends push messages through the browser vendor's push service (e.g., Google FCM, Mozilla autopush). This requires internet access from the ARC VISION server to these push endpoints.

### MQTT

If an [MQTT broker](/integrations/mqtt) is configured, ARC VISION maintains a connection to the broker's host and port. This is typically a local network connection, but will require internet if you use a cloud-hosted MQTT broker.

### DeepStack / CodeProject.AI

When using the [DeepStack detector plugin](/configuration/object_detectors), ARC VISION sends images to the configured API endpoint for inference. This is typically local but depends on where the service is hosted.

## WebRTC (STUN)

For [WebRTC live streaming](/configuration/live), ARC VISION uses STUN for NAT traversal:
...
## Running ARC VISION Offline

To run ARC VISION in an air-gapped or offline environment:

1. **Pre-download models**: Start ARC VISION with internet access once with all desired features enabled. Models will be cached in `/config/model_cache/`.
...
Copy the file into your ARC VISION config volume as `/config/model_cache/MobileNet/mobilenet_v2_weights.h5`, keeping that exact filename, then set the environment variable `TF_KERAS_MOBILENET_V2_WEIGHTS_URL` in your Docker compose file to the URL above and restart ARC VISION.

The variable must be set even though the URL is never contacted. If it is unset, ARC VISION ignores the copied file and asks Keras to download the weights instead.
