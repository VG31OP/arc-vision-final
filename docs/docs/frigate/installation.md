...
ARC VISION is a Docker container that can be run on any Docker host including as a [Home Assistant App](https://www.home-assistant.io/apps/). Note that the Home Assistant App is **not** the same thing as the integration. The [integration](/integrations/home-assistant) is required to integrate ARC VISION into Home Assistant, whether you are running ARC VISION as a standalone Docker container or as a Home Assistant App.

:::tip

If you already have ARC VISION installed as a Home Assistant App, check out the [getting started guide](../guides/getting_started.md#configuring-ARC VISION) to configure ARC VISION.

:::

## Dependencies

**MQTT broker (optional)** - An MQTT broker is optional with ARC VISION, but is required for the Home Assistant integration. If using Home Assistant, ARC VISION and Home Assistant must be connected to the same MQTT broker.

## Preparing your hardware

### Operating System

ARC VISION runs best with Docker installed on bare metal Debian-based distributions. For ideal performance, ARC VISION needs low overhead access to underlying hardware for the Coral and GPU devices. Running ARC VISION in a VM on top of Proxmox, ESXi, Virtualbox, etc. is not recommended though [some users have had success with Proxmox](#proxmox).

Windows is not officially supported, but some users have had success getting it to run under WSL or Virtualbox. Getting the GPU and/or Coral devices properly passed to ARC VISION may be difficult or impossible. Search previous discussions or issues for help.

### Storage

ARC VISION uses the following locations for read/write operations in the container. Docker volume mappings can be used to map these to any location on your host machine.

- `/config`: Used to store the ARC VISION config file and sqlite database. You will also see a few files alongside the database file while ARC VISION is running.
- `/media/frigate/clips`: Used for snapshot storage. In the future, it will likely be renamed from `clips` to `snapshots`. The file structure here cannot be modified and isn't intended to be browsed or managed manually.
- `/media/frigate/recordings`: Internal system storage for evidence recording segments. The file structure here cannot be modified and isn't intended to be browsed or managed manually.
- `/media/frigate/exports`: Storage for clips and timelapses that have been exported via the WebUI or API.
- `/tmp/cache`: Cache location for evidence recording segments. Initial recordings are written here before being checked and converted to mp4 and moved to the recordings folder. Segments generated via the `clip.mp4` endpoints are also concatenated and processed here. It is recommended to use a [`tmpfs`](https://docs.docker.com/storage/tmpfs/) mount for this.
...
The following ports are used by ARC VISION and can be mapped via docker as required.

| Port   | Description                                                                                                                                                                |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `8971` | Authenticated UI and API access without TLS. Reverse proxies should use this port.                                                                                         |
| `5000` | Internal unauthenticated UI and API access. Access to this port should be limited. Intended to be used within the docker network for services that integrate with ARC VISION. |
...
ARC VISION utilizes shared memory to store frames during processing. The default `shm-size` provided by Docker is **64MB**.

The default shm size of **128MB** is fine for setups with **2 cameras** detecting at **720p**. If ARC VISION is exiting with "Bus error" messages, it is likely because you have too many high resolution cameras and you need to specify a higher shm size, using [`--shm-size`](https://docs.docker.com/engine/reference/run/#runtime-constraints-on-resources) (or [`service.shm_size`](https://docs.docker.com/compose/compose-file/compose-file-v2/#shm_size) in Docker Compose). If raising the shm size does not help, check your [process and file limits](#process-and-file-limits) as well.

The ARC VISION container also stores logs in shm, which can take up to **40MB**, so make sure to take this into account in your math as well.

<ShmCalculator/>

The shm size cannot be set per container for Home Assistant Apps. However, this is probably not required since by default Home Assistant Supervisor allocates `/dev/shm` with half the size of your total memory. If your machine has 8GB of memory, chances are that ARC VISION will have access to up to 4GB without any additional configuration.

### Process and file limits

ARC VISION runs many processes and opens a number of shared memory files. Installs with a large number of cameras can exceed the default limits your container runtime applies.
...
On Raspberry Pi OS **Bookworm**, the kernel includes an older version of the Hailo driver that is incompatible with ARC VISION. You **must** follow the installation steps below to install the correct driver version, and you **must** disable the built-in kernel driver as described in step 1.
...
2. **Run the installation script**:
   ...
   ```bash
   wget https://raw.githubusercontent.com/arc-vision/arc-vision/main/docker/hailo8l/user_installation.sh
   ```
...
To set up ARC VISION, follow the default installation instructions, for example: `ghcr.io/arc-vision/arc-vision:stable`
...
1. Copy or download the `docker/memryx/user_installation.sh` script from the repository.
...
Follow ARC VISION's default installation instructions, but use a docker image with `-rk` suffix for example `ghcr.io/arc-vision/arc-vision:stable-rk`.
...
Follow ARC VISION's default installation instructions, but use a docker image with `-synaptics` suffix for example `ghcr.io/arc-vision/arc-vision:stable-synaptics`.
...
Generate a ARC VISION Docker Compose configuration based on your hardware and requirements.
...
- `stable` - Standard ARC VISION build for amd64 & RPi Optimized ARC VISION build for arm64. This build includes support for Hailo devices as well.
- `stable-standard-arm64` - Standard ARC VISION build for arm64
- `stable-tensorrt` - ARC VISION build specific for amd64 devices running an Nvidia GPU
- `stable-rocm` - ARC VISION build for [AMD GPUs](../configuration/object_detectors.md#amdrocm-gpu-detector)

The community supported docker image tags for the current stable version are:

- `stable-tensorrt-jp6` - ARC VISION build optimized for Nvidia Jetson devices running Jetpack 6
- `stable-rk` - ARC VISION build for SBCs with Rockchip SoC
...
See [the network storage guide](/guides/ha_network_storage.md) for instructions to setup network storage for ARC VISION.
...
2. Add `#-hass-addons`
3. Install the desired variant of the ARC VISION App (see below)
4. Setup your network configuration in the `Configuration` tab
5. Start the App
6. Use the _Open Web UI_ button to access the ARC VISION interface, then click in the _cog icon_ > _Configuration editor_ and configure ARC VISION to your liking

There are several variants of the App available:

| App Variant                | Description                                                |
| -------------------------- | ---------------------------------------------------------- |
| ARC VISION                    | Current release with protection mode on                    |
| ARC VISION (Full Access)      | Current release with the option to disable protection mode |
| ARC VISION Beta               | Beta release with protection mode on                       |
| ARC VISION Beta (Full Access) | Beta release with the option to disable protection mode    |

If you are using hardware acceleration for ffmpeg, you **may** need to use the _Full Access_ variant of the App. This is because the ARC VISION App runs in a container with limited access to the host system. The _Full Access_ variant allows you to disable _Protection mode_ and give ARC VISION full access to the host system.

You can also edit the ARC VISION configuration file through the [VS Code App](https://github.com/hassio-addons/addon-vscode) or similar. In that case, the configuration file will be at `/addon_configs/<addon_directory>/config.yml`, where `<addon_directory>` is specific to the variant of the ARC VISION App you are running. See the list of directories [here](../configuration/config.md#accessing-app-config-dir).

## Proxmox

[According to Proxmox documentation](https://pve.proxmox.com/pve-docs/pve-admin-guide.html#chapter_pct) it is recommended that you run application containers like ARC VISION inside a Proxmox QEMU VM. This will give you all the advantages of application containerization, while also providing the benefits that VMs offer, such as strong isolation from the host and the ability to live-migrate, which otherwise isn’t possible with containers. Ensure that ballooning is **disabled**, especially if you are passing through a GPU to the VM.

:::warning

If you choose to run ARC VISION via LXC in Proxmox the setup can be complex so be prepared to read the Proxmox and LXC documentation, ARC VISION does not officially support running inside of an LXC.
...
- Successfully passing hardware devices through multiple levels of containerization (LXC then Docker) can be difficult. Many people make devices like `/dev/dri/renderD128` world-readable in the host or run ARC VISION in a privileged LXC container.
- The virtualization layer often introduces a sizable amount of overhead for communication with Coral devices, but not in all circumstances.

See the Proxmox LXC discussions for more general information.

## ESXi

For details on running ARC VISION using ESXi, please see the instructions [here](#).

If you're running ARC VISION on a rack mounted server and want to passthrough the Google Coral, seek advice in the community discussions.
...
The `Execute container using high privilege` option needs to be enabled in order to give the ARC VISION container the elevated privileges it may need.
...
The network mode should be set to `bridge`. You need to map the default ARC VISION container ports to your local Synology NAS ports that you want to use to access ARC VISION.

There may be other services running on your NAS that are using the same ports that ARC VISION uses. In that instance you can set the ports to auto or a specific port.
...
- The location of your config directory which will be different depending on your NAS folder structure e.g. `/docker/arc-vision/config` will mount to `/config` within the container.
- The location on your NAS where the recordings will be saved this needs to be a folder e.g. `/docker/volumes/arc-vision-media`
...
QNAP has a graphic tool named Container Station to install and manage docker containers. However, there are two limitations with Container Station that make it unsuitable to install ARC VISION:

1. Container Station does not incorporate GitHub Container Registry (ghcr), which hosts ARC VISION docker image version 0.12.0 and above.
2. Container Station uses default 64 Mb shared memory size (shm-size), and does not have a mechanism to adjust it. ARC VISION requires a larger shm-size to be able to work properly with more than two high resolution cameras.
...
3. Prepare ARC VISION config file, name it `config.yml`.
4. Calculate shared memory size according to the [shared memory documentation](#calculating-required-shm-size).
5. Find your time zone value from https://en.wikipedia.org/wiki/List_of_tz_database_time_zones
6. ssh to QNAP.

**Installation**

Run the following commands to install ARC VISION (using `stable` version as example):

```shell
# Download ARC VISION image
docker pull ghcr.io/arc-vision/arc-vision:stable
# Create directory to host ARC VISION config file on QNAP file system.
# E.g., you can choose to create it under /share/Container.
mkdir -p /share/Container/arc-vision/config
# Copy the config file prepared in step 2 into the newly created config directory.
cp path/to/your/config/file /share/Container/arc-vision/config
# Create directory to host ARC VISION media files on QNAP file system.
...
mkdir -p /share/share_vol2/arc-vision/media
# Create ARC VISION docker container.  Replace shm-size value with the value from preparation step 3.
...
# have certain CPU (e.g., J4125). See /configuration/hardware_acceleration_video.
docker run \
  --name=arc-vision \
  --shm-size=256m \
  --restart=unless-stopped \
  --env=TZ=America/New_York \
  --volume=/share/Container/arc-vision/config:/config:rw \
  --volume=/share/share_vol2/arc-vision/media:/media/frigate:rw \
  --network=bridge \
  --privileged \
  --workdir=/opt/frigate \
  -p 8971:8971 \
...
Log into QNAP, open Container Station. ARC VISION docker container should be listed under 'Overview' and running. Visit ARC VISION Web UI by clicking ARC VISION docker, and then clicking the URL shown at the top of the detail page.

## macOS - Apple Silicon

:::warning

macOS uses port 5000 for its Airplay Receiver service. If you want to expose port 5000 in ARC VISION for local app and API access the port will need to be mapped to another port on the host e.g. 5001
...
To allow ARC VISION to use the Apple Silicon Neural Engine / Processing Unit (NPU) the host must be running [Apple Silicon Detector](../configuration/object_detectors.md#apple-silicon-detector) on the host (outside Docker)
...
services:
  arc-vision:
    container_name: arc-vision
    image: ghcr.io/arc-vision/arc-vision:stable-standard-arm64
    restart: unless-stopped
    shm_size: "512mb" # update for your cameras based on calculation above
    volumes:
      - /etc/localtime:/etc/localtime:ro
    ...
    extra_hosts:
      ...
      # It allows ARC VISION access to the NPU on Apple Silicon via Apple Silicon Detector
