...
When using a custom model that natively detects faces, ensure that `face` is added to the list of objects to track either globally or for a specific camera. This will allow face detection to run at the same time as object detection and be more efficient.
...
ARC VISION needs to first detect a `person` before it can detect and recognize a face.

:::

### Face Recognition

ARC VISION has support for two face recognition model types:

- **small**: ARC VISION will run a FaceNet embedding model to recognize faces, which runs locally on the CPU. This model is optimized for efficiency and is not as accurate.
- **large**: ARC VISION will run a large ArcFace embedding model that is optimized for accuracy. It is only recommended to be run when an integrated or dedicated GPU / NPU is available.
...
Like the other real-time processors in ARC VISION, face recognition runs on the camera stream defined by the `detect` role in your config. To ensure optimal performance, select a suitable resolution for this stream in your camera's firmware that fits your specific scene and requirements.
...
1. **Enable face recognition** in your configuration and restart ARC VISION.
2. **Upload one face** using the **Add Face** button's wizard in the Face Library section of the ARC VISION interface. Read below for the best practices on expanding your training set.
3. When ARC VISION detects and attempts to recognize a face, it will appear in the **Train** tab of the Face Library, along with its associated recognition confidence.
...
For a step-by-step narrative of these best practices (and the same principles applied to state and object classification), see the ARC VISION Tips: Best Practices for Training community discussion.
...
### How ARC VISION Decides Who a Person Is

Recognition does not happen one frame at a time. While a `person` is in view, ARC VISION runs face recognition on many frames, not just a single frame. The final `sub_label` is decided from all of those attempts together, weighted by the area of each face (larger, closer faces count more), not from any single frame.

This has a few practical consequences:

- A handful of wrong guesses on blurry or distant frames usually do not change the result. If ARC VISION sees a person as "Tom, Tom, Sam, Tom, Tom," it will still conclude the person was Tom.
...
1. Enable debug logs to see exactly what ARC VISION is doing.
   - Enable debug logs for face recognition by adding `frigate.data_processing.real_time.face: debug` to your `logger` configuration. Restart ARC VISION after this change.
...
2. Ensure `person` is being _detected_. A `person` will automatically be scanned by ARC VISION for a face. Any detected faces will appear in the Recent Recognitions tab in the ARC VISION interface's Face Library.

   If you are using a model that natively detects `face`:
   ...
   If your model does **not** natively detect `face`:
...
<FaqItem id="arc-vision-misidentified-a-face-can-i-tell-it-that-a-face-is-not-a-specific-person" question={'ARC VISION misidentified a face. Can I tell it that a face is "not" a specific person?'}>
...
No, using another face recognition service will interfere with ARC VISION's built in face recognition. When using double-take the sub_label feature must be disabled if the built in face recognition is also desired.
...
ARC VISION does not store anything in its database related to face recognition. You can simply delete all of your faces through the ARC VISION interface or remove the contents of the `/media/frigate/clips/faces` directory.
