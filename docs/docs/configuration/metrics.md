...
ARC VISION exposes Prometheus metrics at the `/api/metrics` endpoint that can be used to monitor the performance and health of your ARC VISION instance. For compatibility with existing dashboards and alerting rules, ARC VISION retains the established `frigate_*` metric naming convention.
...
Metrics are available at `/api/metrics` by default. No additional ARC VISION configuration is required to expose them.
...
These gauges report the operating system's figures for the whole filesystem (the same numbers as `df`), not ARC VISION's own recording footprint. For how this differs from the recordings usage shown in the UI, see [Understanding storage usage](/configuration/record#understanding-storage-usage).
...
To scrape metrics from ARC VISION, add the following to your Prometheus configuration:
...
You can use these metrics to create Grafana dashboards to monitor your ARC VISION instance. Here's an example of metrics you might want to track:
...
The metrics exposed by ARC VISION use the following Prometheus metric types:
