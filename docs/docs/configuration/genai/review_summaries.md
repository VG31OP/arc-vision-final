...
Generative AI can be used to automatically generate structured summaries of incident review items. These summaries will show up in ARC VISION's native notifications as well as in the UI. Generative AI can also be used to take a collection of summaries over a period of time and provide a report, which may be useful to get a quick report of everything that happened while out for some amount of time.

Requests for a summary are requested automatically to your AI provider for alert incident review items when the activity has ended, they can also be optionally enabled for detections as well.

Generative AI incident review summaries can also be toggled dynamically for a [camera via MQTT](/integrations/mqtt#frigatecamera_namereview_descriptionsset).
...
By default, incident review summaries use preview images (cached preview frames) which have a lower resolution but use fewer tokens per image. For better image quality and more detailed analysis, configure ARC VISION to extract frames directly from recordings at a higher resolution.
...
Along with the concern of suspicious activity or immediate threat, you may have concerns such as animals in your garden or a gate being left open. Configure these concerns so that incident review summaries will make note of them if the activity requires additional review.
...
By default, incident review summaries are generated in English. Configure ARC VISION to generate summaries in your preferred language by setting the `preferred_language` option.
...
Along with individual review item summaries, Generative AI can also produce a single report of incident review items from all cameras marked "suspicious" over a specified time period (for example, a daily summary of suspicious activity while you're on vacation).
