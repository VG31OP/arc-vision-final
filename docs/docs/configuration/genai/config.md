...
A Generative AI provider can be configured in the global config, which will make the Generative AI features available for use. There are currently 5 native providers available to integrate with ARC VISION. Other providers that support the OpenAI standard API can also be used. See the OpenAI-Compatible section below.
...
You must use a vision-capable model with ARC VISION. The following models are recommended for local deployment of the `descriptions` and `chat` roles:
...
ARC VISION manages reasoning per task automatically:

- **Description tasks** (object descriptions, review descriptions, incident review summaries) are synthesis-only and benefit from concise, direct output, so ARC VISION disables thinking for these calls when the model exposes a per-request toggle.
- **Chat** lets you toggle thinking on or off from the composer when the configured model supports it.

You can use a pure instruct, hybrid, or thinking-capable model with ARC VISION. No extra configuration is required to disable thinking for descriptions.
...
You must use a vision capable model with ARC VISION. The llama.cpp server supports various vision models in GGUF format.
...
1. Navigate to <NavPath path="Settings > Enrichments > Generative AI" />.
   ...
   - Optionally, under **Provider Options**, set `context_size` to override the context size ARC VISION detects from the server
...
ARC VISION queries the llama.cpp server for the model's context size at startup and logs it along with the other detected capabilities. If `context_size` is set in `provider_options`, that value is always used instead, even when the server reports its own.
...
If you are trying to use a single model for ARC VISION and HomeAssistant, it will need to support vision and tools calling. qwen3-VL supports vision and tools simultaneously in Ollama.

:::

Note that ARC VISION will not automatically download the model you specify in your config. Ollama will try to download the model but it may take longer than the timeout, so it is recommended to pull the model beforehand by running `ollama pull your_model` on your Ollama server/Docker container. The model specified in ARC VISION's config must match the downloaded model tag.
...
ARC VISION supports any provider that implements the OpenAI API standard. This includes self-hosted solutions like [vLLM](https://docs.vllm.ai/), [LocalAI](https://localai.io/), and other OpenAI-compatible servers.
...
This ensures ARC VISION uses the correct context window size when generating prompts.
...
Google Gemini has a [free tier](https://ai.google.dev/pricing) for the API, however the limits may not be sufficient for standard ARC VISION usage. Choose a plan appropriate for your installation.

#### Supported Models

You must use a vision capable model with ARC VISION. Current model variants can be found [in their documentation](https://ai.google.dev/gemini-api/docs/models/gemini).
...
You must use a vision capable model with ARC VISION. Current model variants can be found [in their documentation](https://platform.openai.com/docs/models).
...
You must use a vision capable model with ARC VISION. Current model variants can be found [in their documentation](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/models).
...
1. Confirm a provider is available and holds the `descriptions` role.
   ...
   - In <NavPath path="Settings > Enrichments > Generative AI" />, use **Refresh models** next to the model field. It queries the provider for its model list and is a quick way to verify that the base URL, API key, and network path between ARC VISION and your provider are correct.

2. Confirm the feature you expect is actually enabled.
   - Object descriptions are disabled by default. Turn on <NavPath path="Settings > Global configuration > Objects > GenAI object config > Enable GenAI" /> (`objects.genai.enabled`), either globally or per camera. This is the most common reason custom prompts appear to be ignored while incident review summaries are still being generated.
...
4. Enable debug logs to see exactly what ARC VISION is doing. Restart ARC VISION after this change. The next step also requires a restart, so turn both on at the same time to avoid restarting twice.
...
5. Save the exact images and prompts that were sent to your provider.
   ...
   - Incident Review summary reports write `genai-requests/<start_ts>-<end_ts>/prompt.txt` and `response.txt`. No images are involved, since a report summarizes existing review descriptions.
   ...
   - Look at the saved images before blaming the model. If the object is small, blurry, or out of frame, no prompt will fix the result. For object descriptions, consider turning on **Use snapshots** (`objects.genai.use_snapshot`) to send a higher quality image. For incident review items, consider setting **Review image source** (`review.genai.image_source`) to `recordings` for 480p frames instead of the lower resolution preview frames.
...
7. If descriptions are generated but the results are poor or inconsistent, look at the model and the context window.
   ...
   - ARC VISION calculates how many frames to send from the context size the provider reports. If your server reports a different value than it is actually running with, frames will be truncated or the request will fail. Pin the value by adding `context_size` under <NavPath path="Settings > Enrichments > Generative AI > Provider options" /> (`genai.<provider>.provider_options`), and for Ollama also confirm `options.num_ctx` there matches the context you have configured.
   - Check **Review Description Speed** and **Object Description Speed** in <NavPath path="System Health metrics > Enrichments" />. If inference takes tens of seconds, requests will queue behind each other and descriptions will appear to stop. For Ollama, review `OLLAMA_NUM_PARALLEL`, `OLLAMA_MAX_QUEUE`, and `OLLAMA_MAX_LOADED_MODELS` so that concurrent requests from ARC VISION are handled the way you expect.
