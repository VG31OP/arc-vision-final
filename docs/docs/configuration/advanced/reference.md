...
detectors:
  ...
  detector_name:
    ...
    # ARC VISION provides many types, see /configuration/object_detectors for more details (default: shown below)
...
networking:
  ...
  ipv6:
    enabled: False
  # Optional: Override ports ARC VISION uses for listening (defaults: shown below)
...
proxy:
  # Optional: Mapping for headers from upstream proxies. Only used if ARC VISION's auth
  # is disabled.
...
model:
  # Required: path to the model. model-service managed URIs reference library-backed models (default: automatic based on detector)
...
# More information about presets at /configuration/ffmpeg_presets
ffmpeg:
  ...
  # If set too low, ARC VISION will retry a connection to the camera's stream too frequently, using up the limited streams some cameras can allow at once
...
detect:
  ...
  min_initialized: 2
  # Optional: Number of frames without a detection before ARC VISION considers an object to be gone. (default: 5x the frame rate)
  max_disappeared: 25
...
face_recognition:
  ...
  # NOTE: This only applies when your model does not natively detect `face`
...
live:
  ...
  # that should be used for live view in ARC VISION WebUI. (default: name of camera)
...
cameras:
  ...
  back:
    ...
    # (except Camera Management). Re-enabling requires a ARC VISION restart.
    enabled: True
    # Optional: camera type used for some ARC VISION features (default: shown below)
