...
Object detection is what allows ARC VISION to identify _what_ is in your camera's view (people, cars, animals, and more) rather than just reacting to pixel changes. When ARC VISION's motion detection finds activity in a frame, that region is sent to an **object detector**, which returns the objects it recognizes along with their location and a confidence score. These detections are what drive tracked objects, alerts, detections, and notifications.

Object detection is computationally intensive, so ARC VISION is designed to run it on a dedicated AI accelerator or GPU rather than the CPU. A **detector** is the specific hardware-and-model backend ARC VISION uses to run inference. Choosing a detector that matches your hardware is one of the most important steps in getting good performance, and the right choice depends on what device ARC VISION is running on.

:::info
ARC VISION supports multiple different detectors that work on different types of hardware:
...
- [ONNX](#onnx): ROCm will automatically be detected and used as a detector in the `-rocm` ARC VISION image when a supported ONNX model is configured.
...
- [ONNX](#onnx): OpenVINO will automatically be detected and used as a detector in the default ARC VISION image when a supported ONNX model is configured.

**Nvidia GPU**

- [ONNX](#onnx): Nvidia GPUs will automatically be detected and used as a detector in the `-tensorrt` ARC VISION image when a supported ONNX model is configured.

**Nvidia Jetson** <CommunityBadge />

- [TensortRT](#nvidia-tensorrt-detector): TensorRT can run on Jetson devices, using one of many default models.
- [ONNX](#onnx): TensorRT will automatically be detected and used as a detector in the `-tensorrt-jp6` ARC VISION image when a supported ONNX model is configured.
...
**Resolution (320x320 vs 640x640):** ARC VISION is optimized for `320x320` models, and `320x320` is the best choice for the vast majority of setups. ARC VISION is specifically designed to compensate for the smaller model by cropping a region of motion from the full frame and zooming into it before running detection, so a `320x320` model is actually _better_ at small and distant objects, not worse. A `640x640` model is slower and uses more resources, and its main benefit is fitting more objects into a single inference when many objects are spread across a large area. Recent versions of ARC VISION have improved support for `640x640` models, but `320x320` remains the recommended starting point for nearly all setups.
...
The best detection accuracy comes from a model trained on images that look like what ARC VISION actually sees: security camera footage cropped to regions of interest. You can train or fine-tune your own model on images like this and run it as a custom model (see the per-detector sections below). For YOLOv9, the `s` (small) variant at `320x320` resolution is a good place to start.

:::

# Officially Supported Detectors

ARC VISION provides a number of builtin detector types. By default, ARC VISION will use a single CPU detector. Other detectors may require additional configuration as described below. When using multiple detectors they will run in dedicated processes, but pull from a common queue of detection requests from across all cameras.
...
_warning: may have compatibility issues after `v0.9.x`_
...
The NPU firmware is loaded by the host kernel and is not part of the ARC VISION image. Everything else the NPU needs is bundled in the container, so host NPU libraries should never be mounted in.

ARC VISION bundles a specific version of Intel's [linux-npu-driver](https://github.com/intel/linux-npu-driver/releases), and the host firmware must come from that release or a newer one. Firmware older than the bundled driver may fail with `MAPPED_INFERENCE_VERSION is NOT compatible with the ELF`, where `Expected` is the version the firmware supports and `received` is the version the bundled compiler produced. Distributions often package older firmware than the driver ARC VISION ships, so check the build date on the host with `sudo dmesg | grep -i vpu` and update it there if needed.
...
The NPU in Apple Silicon can't be accessed from within a container, so the [Apple Silicon detector client](https://github.com/arc-vision/apple-silicon-detector) must first be setup. It is recommended to use the ARC VISION docker image with `-standard-arm64` suffix, for example `ghcr.io/arc-vision/arc-vision:stable-standard-arm64`.

### Setup {#setup-apple-silicon}

1. Setup the [Apple Silicon detector client](https://github.com/arc-vision/apple-silicon-detector) and run the client
2. Configure the detector in ARC VISION and startup ARC VISION
...
Support for AMD GPUs is provided using the [ONNX detector](#onnx). In order to utilize the AMD GPU for object detection use a ARC VISION docker image with `-rocm` suffix, for example `ghcr.io/arc-vision/arc-vision:stable-rocm`.

### Docker settings for GPU access

ROCm needs access to the `/dev/kfd` and `/dev/dri` devices. When docker or ARC VISION is not run under root then also `video` (and possibly `render` and `ssl/_ssl`) groups should be added.
...
For the rocm ARC VISION build there is some automatic detection:
...
- first make sure that rocm environment is running properly by running `/opt/rocm/bin/rocminfo` in the ARC VISION container -- it should list both the CPU and the GPU with their properties
...
- if things are not working check the ARC VISION docker logs

#### Figuring out if AMD/ROCm is working and found your GPU

```bash
$ docker exec -it arc-vision /opt/rocm/bin/rocminfo
```
...
```bash
$ docker exec -it arc-vision /bin/bash -c '(unset HSA_OVERRIDE_GFX_VERSION && /opt/rocm/bin/rocminfo |grep gfx)'
```
...
2. Startup ARC VISION with the onnx detector configured, the main object detection model will be converted to mxr format and cached in the config directory.
...
ONNX is an open format for building machine learning models, ARC VISION supports running ONNX models on CPU, OpenVINO, ROCm, and TensorRT. On startup ARC VISION will automatically try to use a GPU if one is available.
...
- **AMD**
  - ROCm will automatically be detected and used with the ONNX detector in the `-rocm` ARC VISION image.

- **Intel**
  - OpenVINO will automatically be detected and used with the ONNX detector in the default ARC VISION image.

- **Nvidia**
  - Nvidia GPUs will automatically be detected and used with the ONNX detector in the `-tensorrt` ARC VISION image.
  - Jetson devices will automatically be detected and used with the ONNX detector in the `-tensorrt-jp6` ARC VISION image.
...
The Deepstack / CodeProject.AI Server detector for ARC VISION allows you to integrate Deepstack and CodeProject.AI object detection capabilities into ARC VISION. CodeProject.AI and DeepStack are open-source AI platforms that can be run on various devices such as the Raspberry Pi, Nvidia Jetson, and other compatible hardware. It is important to note that the integration is performed over the network, so the inference times may not be as fast as native ARC VISION detectors, but it still provides an efficient and reliable solution for object detection and tracking.

### Setup {#setup-deepstack}

To get started with CodeProject.AI, visit their [official website](https://www.codeproject.com/Articles/5322557/CodeProject-AI-Server-AI-the-easy-way) to follow the instructions to download and install the AI server on your preferred device. Detailed setup instructions for CodeProject.AI are outside the scope of the ARC VISION documentation.

To integrate CodeProject.AI into ARC VISION, configure the detector as follows:
...
To verify that the integration is working correctly, start ARC VISION and observe the logs for any error messages related to CodeProject.AI. Additionally, you can check the ARC VISION web interface to see if the objects detected by CodeProject.AI are being displayed and tracked properly.

# Community Supported Detectors

## MemryX MX3

This detector is available for use with the MemryX MX3 accelerator M.2 module. ARC VISION supports the MX3 on compatible hardware platforms, providing efficient and high-performance object detection.
...
Nvidia Jetson devices may be used for object detection using the TensorRT libraries. Due to the size of the additional libraries, this detector is only provided in images with the `-tensorrt-jp6` tag suffix, e.g. `ghcr.io/arc-vision/arc-vision:stable-tensorrt-jp6`. This detector is designed to work with Yolo models for object detection.
...
- All models are automatically downloaded and stored in the folder `config/model_cache/rknn_cache`. After upgrading ARC VISION, you should remove older models to free up space.
...
- Run `docker exec <container_id> python3 /opt/conv2rknn.py`. If the conversion was successful, the rknn models will be placed in `config/model_cache/rknn_cache`.
