...
The Incident Review page of the ARC VISION interface is for quickly reviewing historical footage of interest from your cameras. _Review items_ are indicated on a vertical timeline and displayed as a grid of previews - bandwidth-optimized, low frame rate, low resolution videos. Hovering over or swiping a preview plays the video and marks it as reviewed. If more in-depth analysis is required, the preview can be clicked/tapped and the full frame rate, full resolution recording is displayed.

incident review items are filterable by date, object type, and camera.

### incident review items vs. tracked objects (formerly "events")

In ARC VISION 0.13 and earlier versions, the UI presented "events". An event was synonymous with a tracked or detected object. In ARC VISION 0.14 and later, a review item is a time period where any number of tracked objects were active.
...
In this scenario, ARC VISION 0.13 and earlier would show 4 "events" in the UI - one for each person, another for the dog, and yet another for the car. You would have had 4 separate videos to watch even though they would have all overlapped.
...
Not every segment of video captured by ARC VISION may be of the same level of interest to you. Video of people who enter your property may be a different priority than those walking by on the sidewalk. For this reason, ARC VISION categorizes incident review items as _alerts_ and _detections_. By default, all person and car objects are considered alerts. You can refine categorization of your incident review items by configuring [required zones](/configuration/zones#restricting-alerts-and-detections-to-specific-zones) for them.

:::note

Alerts and detections categorize the tracked objects in incident review items, but ARC VISION must first detect those objects with your configured object detector (Coral, OpenVINO, etc). By default, the object tracker only detects `person`. Setting `labels` for `alerts` and `detections` does not automatically enable detection of new objects. To detect more than `person`, you should add more labels via <NavPath path="Settings > Global configuration > Objects" /> or <NavPath path="Settings > Camera configuration > Objects" /> and select your camera. Alternatively, add the following to your config:
...
See the [objects documentation](objects.md) for the list of objects that ARC VISION's default model tracks.
:::
...
## Restricting incident review items to specific zones

By default a review item will be created if any `review -> alerts -> labels` and `review -> detections -> labels` are detected anywhere in the camera frame. You will likely want to configure incident review items to only be created when the object enters an area of interest, [see the security zone docs for more information](./zones.md#restricting-alerts-and-detections-to-specific-zones)
