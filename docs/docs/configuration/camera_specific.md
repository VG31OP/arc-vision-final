...
Many cameras support encoding options which greatly affect the live surveillance experience, see the [Live Surveillance](/configuration/live) page for more info.
      ...
      inputs:
        - path: rtsp://USERNAME:PASSWORD@CAMERA-IP/H264/ch1/main/av_stream # <----- Update for your camera
          roles:
            ...
            - record
    detect:
      width: # <- optional, by default ARC VISION tries to automatically detect resolution
      height: # <- optional, by default ARC VISION tries to automatically detect resolution
```
...
[Some users have reported](#) that newer Hikvision cameras require adjustments to the security settings:
...
| 6MP or higher     | Latest (ex: Duo3, CX-8##) | http-flv with ffmpeg 8.0, or rtsp | This uses the new http-flv-enhanced over H265 which requires ffmpeg 8.0 (ARC VISION's default) |
| 6MP or higher     | Older (ex: RLC-8##)       | rtsp                              |                                                                                             |

ARC VISION works much better with newer Reolink cameras that are setup with the below options:
...
The wizard covers standard single-camera setups. For two way talk, cameras connected through a Reolink NVR, or audio transcoding for WebRTC live surveillance, configure the camera manually as shown below.

#### Manual configuration

Based on community experience, the http video streams seem to be the most reliable for Reolink.
...
Ensure [HTTP is enabled](https://support.reolink.com/articles/360003452893-How-to-Access-Reolink-Cameras-NVRs-Home-Hub-Locally-via-Web-Browsers/) in the camera's advanced network settings. To use two way talk with ARC VISION, see the [Live Surveillance documentation](/configuration/live#two-way-talk).
...
To use a USB camera (webcam) with ARC VISION, the recommendation is to use go2rtc's [FFmpeg Device](https://github.com/AlexxIT/go2rtc?tab=readme-ov-file#source-ffmpeg-device) support:

- Preparation outside of ARC VISION:
  ...
  - If using ARC VISION in a container (e.g. Docker on TrueNAS), ensure you have USB Passthrough support enabled, along with a specific Host Device (`/dev/video0`) + Container Device (`/dev/video0`) listed.

- In your ARC VISION Configuration File, add the go2rtc stream and roles as appropriate:
