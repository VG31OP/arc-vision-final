...
ARC VISION uses the bundled go2rtc to power a number of key features:
...
Configure go2rtc to connect to your camera by adding the stream you want to use for live surveillance. Avoid changing any other parts of your config at this step. Note that go2rtc supports [many different stream types](https://github.com/AlexxIT/go2rtc/tree/v1.9.14#module-streams), not just rtsp.

:::tip

For the best experience, set the stream name under `go2rtc` to match the name of your camera so that ARC VISION will automatically map it and be able to use better live surveillance options for the camera.

See [the live surveillance docs](/configuration/live#setting-streams-for-live-ui) for more information.
...
Navigate to <NavPath path="Settings > System > go2rtc Streams" /> and click **Add stream**. Give the stream a name (use the camera's name so ARC VISION can auto-map it - for example, if your camera's name is `back`, use `back` as the go2rtc stream name), then paste the camera's stream URL into the **Source** field. Save the section.
...
After adding this to the config, restart ARC VISION and try to watch the live stream for a single camera by clicking on it from the dashboard. It should look much clearer and more fluent than the original jsmpeg stream.

### Next steps

1. If the stream you added to go2rtc is also used by ARC VISION for the `record` or `detect` role, you can migrate your config to pull from the RTSP restream to reduce the number of connections to your camera as shown [here](/configuration/restream#reduce-connections-to-camera).
...
To export camera streams to HomeKit, ARC VISION must be configured in docker to use `host` networking mode. HomeKit settings are stored in `/config/go2rtc_homekit.yml` rather than in your ARC VISION config, and are edited through the go2rtc config editor at `http://<arc_vision_host>:1984/editor.html`. Pairings are saved back to that file automatically.
