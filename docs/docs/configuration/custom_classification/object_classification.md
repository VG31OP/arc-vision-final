...
Creating and training the model is done within the ARC VISION interface using the `Classification` page. The process consists of two steps:

### Step 1: Name and Define

Enter a name for your model, select the object label to classify (e.g., `person`, `dog`, `car`), choose the classification type (sub label or attribute), and define your classes. ARC VISION will automatically include a `none` class for objects that don't fit any specific category.
...
- **Preprocessing**: Ensure examples reflect object crops similar to ARC VISION's boxes; keep the subject centered.
...
Enable debug logs for classification models by adding `frigate.data_processing.real_time.custom_classification: debug` to your `logger` configuration. These logs are verbose, so only keep this enabled when necessary. Restart ARC VISION after this change.
