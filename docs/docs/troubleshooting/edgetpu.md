...
Using `lsusb` or checking the hardware page in HA OS will show as `1a6e:089a Global Unichip Corp.` until ARC VISION runs an inference using the coral. So don't worry about the identification until after ARC VISION has attempted to detect the coral.

:::

If the coral does not initialize then ARC VISION can not interface with it. Some common reasons for the USB based Coral not initializing are:
...
- When running ARC VISION in a VM, Proxmox lxc, etc. you must ensure both device IDs are mapped.
- When running through the Home Assistant OS you may need to run the Full Access variant of the ARC VISION App with the _Protection mode_ switch disabled so that the coral can be accessed.
...
7. Start the ARC VISION container. Everything should work now!

### QNAP NAS

QNAP NAS devices, such as the TS-253A, may use connected Coral TPU devices if [QuMagie](https://www.qnap.com/en/software/qumagie) is installed along with its QNAP AI Core extension. If any of the features (`facial recognition`, `object recognition`, or `similar photo recognition`) are enabled, Container Station applications such as `ARC VISION` or `CodeProject.AI Server` will be unable to initialize the TPU device in use.
...
3. Manually start the QNAP AI Core extension after ARC VISION has fully started (not recommended).
...
2. Running ARC VISION in a VM may cause communication with the device to be lost and need to be reset.
