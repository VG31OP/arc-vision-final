...
ARC VISION includes built-in memory profiling using [memray](https://bloomberg.github.io/memray/) to help diagnose memory issues. This feature allows you to profile specific ARC VISION modules to identify memory leaks, excessive allocations, or other memory-related problems.
...
docker run -e FRIGATE_MEMRAY_MODULES="frigate.embeddings" \
   ...
   --name arc-vision <arc_vision_image>
```

### Module Names

ARC VISION processes are named using a module-based naming scheme. Common module names include:
...
2. **Automatic HTML Generation**: On normal process exit, ARC VISION automatically:
...
- Run `memray` inside the ARC VISION container:

```bash
docker-compose exec arc-vision memray flamegraph /config/memray_reports/<module_name>.bin
# or
...
- Check that memray is properly installed (included by default in ARC VISION)
