...
These are the MQTT messages generated by ARC VISION. For compatibility with existing tooling and automations, ARC VISION retains the `frigate` default `topic_prefix`, which can be changed in the config file.
...
"online": published once ARC VISION is running and has published its initial state. Note that this is published on every connection to the broker, so it is republished if the broker restarts or the connection drops and recovers, without ARC VISION itself restarting.
"stopped": published when ARC VISION is stopped normally
"offline": published automatically by the MQTT broker if ARC VISION disconnects unexpectedly (via MQTT Will Message)

### `frigate/restart`

Causes ARC VISION to exit. Docker should be configured to automatically restart the container on exit.

### `frigate/events`

Message published for each changed tracked object. The first message is published when the tracked object is no longer marked as a false_positive. When ARC VISION finds a better snapshot of the tracked object or when a security zone change occurs, it will publish a message with the same id. When the tracked object ends, a final message is published with `end_time` set.
...
Only available when notifications are enabled in the config. Not persisted across ARC VISION restarts.

### `frigate/notifications/state`

Topic with current state of notifications. Published values are `ON` and `OFF`.

## ARC VISION Camera Topics
...
Publishes the count of objects for the security zone for use as a sensor in Home Assistant.
`all` can be used as the object_name for the count of all objects for the security zone.

### `frigate/<zone_name>/<object_name>/active`

Publishes the count of active objects for the security zone for use as a sensor in Home
...
Topic to turn ARC VISION's processing of a camera on or off at runtime. Expected values are `ON` and `OFF`. The change is persisted across ARC VISION restarts (see [Runtime toggle persistence](/configuration/live#runtime-toggle-persistence)). To permanently change the configured value, use **Settings → Global configuration → Camera management** in the ARC VISION interface. See [Camera state](/configuration/live#camera-state) for the difference between turning a camera off and disabling it.
...
Topic to turn object detection for a camera on and off. Expected values are `ON` and `OFF`. The change is persisted across ARC VISION restarts (see [Runtime toggle persistence](/configuration/live#runtime-toggle-persistence)).
...
Topic to turn audio detection for a camera on and off. Expected values are `ON` and `OFF`. The change is persisted across ARC VISION restarts (see [Runtime toggle persistence](/configuration/live#runtime-toggle-persistence)).
...
`ON` is ignored unless audio transcription is enabled in the config for the camera. Unlike the other camera toggles, this one is not persisted across ARC VISION restarts.
...
Topic to turn recordings for a camera on and off. Expected values are `ON` and `OFF`. The change is persisted across ARC VISION restarts (see [Runtime toggle persistence](/configuration/live#runtime-toggle-persistence)).
...
Topic to turn snapshots for a camera on and off. Expected values are `ON` and `OFF`. The change is persisted across ARC VISION restarts (see [Runtime toggle persistence](/configuration/live#runtime-toggle-persistence)).
...
`ON` is ignored unless notifications are enabled in the config for the camera. This is not persisted across ARC VISION restarts. It is the same control the UI labels **Suspend until restart**.
