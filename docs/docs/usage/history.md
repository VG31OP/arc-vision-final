...
**History** is ARC VISION's full-resolution recording viewer. Unlike Live, Review, and Explore, there is no menu item for it. You reach it from within another view, then scrub the timeline, switch cameras, inspect a tracked object's lifecycle, and export or share any moment.
...
Use the **Back** button to return where you came from, or the **Live** button to jump to the current camera's live surveillance.

:::tip

If you see **"No recordings found for this time"**, the most common causes are: recording was not enabled for that camera at the time of the event; the retention window has since expired and those segments were removed; or storage ran low and ARC VISION deleted them early to free space. See [Recording](/configuration/record) to verify your retention settings.
...
- **Timeline**: a scrubbable vertical timeline of the selected camera. Horizontal lines down the center represent motion, with longer lines indicating more motion at that moment. incident review items are marked as shaded areas (**red** for alerts, **orange** for detections), and sections with no colored background are times when no recording exists.
- **Events**: a scrollable list of the camera's incident review items for the time range; clicking one seeks the player to it.
...
Expanding a card reveals the **lifecycle** of each tracked object: a row for each significant moment (detected, entered a security zone, became active, became stationary, left, and so on), with a progress line that follows the current playback position. Hovering a row shows that moment's score, ratio, and area, and clicking a row seeks the video to that exact timestamp.
...
- **Share Timestamp**: generate a link to the current moment (or a custom timestamp) to share with another ARC VISION user. This is an internal link, not a public share URL.
...
When [Generative AI review](/configuration/genai/genai_review) is configured, ARC VISION can generate a title, description, and threat classification for incident review items and surface them as you scrub through History. A review item that has an AI summary exposes its details in a few places:
