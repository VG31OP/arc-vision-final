...
Automatically generates "ARC VISION interface" tab content for documentation files based on the YAML config examples already in the docs.
...
Run from the repository root. The script imports ARC VISION's Python config models directly, so the `frigate` package must be importable:

```bash
# From repo root -- no extra install needed if your environment can import ARC VISION
