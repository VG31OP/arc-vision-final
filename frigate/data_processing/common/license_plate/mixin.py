...
class LicensePlateProcessingMixin:
    ...
    def _detect_license_plate(
        ...
        """
        Use a lightweight YOLOv9 model to detect license plates for users
        without the model-service integration.
    ...
    def lpr_process(
        ...
        # dedicated LPR cam without the model-service integration
        if dedicated_lpr:
            id = "dedicated-lpr"
            ...
            # don't run for non car/motorcycle or non license plate objects
            # in dedicated LPR workflows with an attached model-service path
            if (
            ...
            if "license_plate" not in self.config.cameras[camera].objects.track:
                ...
                # don't run for object without attributes if this isn't a
                # dedicated LPR workflow with the model-service integration
                if (
                ...
                # we are using a dedicated LPR workflow with the model-service integration
        ...
        # save the best snapshot for dedicated LPR cams not using the
