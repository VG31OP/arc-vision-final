...
def set_jwt_cookie(response: Response, cookie_name, encoded_jwt, max_age, secure):
    ...
    # iframes, breaking embedded views such as the Home Assistant ARC VISION card.
    # CSRF is instead mitigated by requiring a custom X-CSRF-TOKEN header, which
    # cross-origin pages cannot set without a CORS preflight that ARC VISION never
...
def auth(request: Request):
    ...
    # this header is set by ARC VISION's nginx proxy, so it cant be spoofed.
